...
function UnevaluatedBracket(v) {
    this.v = v;
}

function parseBracket(s, b) {
    var ss = s;
    s = s.trim();
    if(s[0] != '[') {
        if(b) {
            var us = s.split(/\s+/)[0];
            var nn = ss.length-s.length + us.length;
            return {v:us, s: ss.substring(nn, s.length), n:nn};
        }
        return {v: s, s: '', n:ss.length};
    }
    s = s.substring(1, s.length);
    var v = [];
    while(s.length > 0 && s.trim()[0] != ']') {
        //console.log(s);
        s = s.trim();
        var a = parseBracket(s.split(/[|\]]/)[0]);
        v.push(a.v);
        s = s.substring(a.n, s.length);
        if(s[0] === '|') {
            s = s.substring(1, s.length);
        }
    ...
    s = s.trim();
    s = s.substring(1, s.length);
    return {v:new UnevaluatedBracket(v), s:s, n: ss.length - s.length};
}
...
function levelsToArray(state) {
    ...
    for (var levelIndex = 0; levelIndex < levels.length; levelIndex++) {
        ...
        if (level[0] == '\n') {
            ...
            if(o.action === 'begincond' || o.action === 'contcond' || o.action === 'elsecond' || o.action === 'evaluate' || o.action === 'goto') {
                o.message = parseGlobal(o.message);
            }
    ...
    for(var i = processedLevels.length-1; i >= 0; i--) {
        switch(processedLevels[i].action) {
        case 'endcond': {
            keys.push(i);
            break;
        }
        case 'contcond': {
            if(!keys.length) {
                consoleError('Mismatched conditionals; behavior may be unpredictable');
            }
            processedLevels[i].dest = keys.pop();
            keys.push(i);
            console.log('link ' + i + ' -> ' + processedLevels[i].dest);
            break;
        }
        case 'elsecond': {
            if(!keys.length) {
        ...
        case 1: {
            if (token == '[') {
                ...
                if (token==='message') {
                    ...
                    i=tokens.length;
                } else if (token==='wintype') {
                    var winIx = findIndexAfterToken(origLine,tokens,i);
                    var winTyp = origLine.substring(winIx).trim();
                    if (winTyp===""){
                        winTyp=" ";
                        //needs to be nonempty or the system gets confused and thinks it's a whole level message rather than an interstitial.
                    }
                    commands.push([token, winTyp]);
                    i++;
                } else {
                    commands.push([token]);
                }
...
function processWinConditions(state) {
    ...
    for (var w in state.winconditions)  {
        newconditions[w] = [];
        for(var i = 0; i < state.winconditions[w].length; i++) {
            var wincondition=state.winconditions[w][i];
            ...
            newconditions[w].push(newcondition);
        }
    }

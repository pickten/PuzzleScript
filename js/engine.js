...
{
    titleMode=(g.curlevel>0||g.curlevelTarget!==null)?1:0;
...
function drawMessageScreen() {
    ...
    var message;
    if (messagetext==="") {
        var leveldat = state.levels[g.curlevel];
...
function loadLevelFromLevelDat(state,leveldat,randomseed) {	
    ...
    titleScreen=false;
    titleMode=(g.curlevel>0||g.curlevelTarget!==null)?1:0;
    titleSelection=(g.curlevel>0||g.curlevelTarget!==null)?1:0;
    titleSelected=false;
    againing=false;
    ...
    if (leveldat.message===undefined) {
        ...
        function cont(n) {
            ...
            loadLevelFromLevelDat(state,state.levels[g.curlevel],randomseed);
        }
        switch(leveldat.action) {
        case 'message': {
        ...
        case 'goto': {
            var ll = firstLevelFrom(computeBracketed(['+level'].concat(leveldat.message)),true);
            console.log('goto --', ll);
            g.condstack = [];
            g.condvalstack = [];
            loadLevelFromTag(state, ll, randomseed);
            break;
        }
        case 'evaluate': {
            leveldat.message.forEach(computeBracket);
            cont();
            break;
        }
        case 'begincond': {
            ...
            for(var i = 0; i < s.length; i++) {
                g.condvalstack.unshift(computeBracket(s[i]));
                o.num++;
            }
            g.condstack.unshift(o);
            console.log('begincond (o, condval)', o, g.condvalstack);
            cont();
            break;
        }
        case 'contcond': {
            m = true;
            s = leveldat.message;
            for(var i = 0; m && i < s.length; i++) {
                console.log('cont matching vals', s[i], 'and', g.condvalstack[i]);
                m = m && matchVals(computeBracket(s[i]), g.condvalstack[i]);
                //console.log('new m is ' + m);
            }
            if(m) {
                g.condstack[0].met = true;
                cont();
            } else {
                cont(leveldat.dest);
            }
        ...
        case 'elsecond': {
            ...
            s = leveldat.message;
            for(var i = 0; m && i < s.length; i++) {
                console.log('matching vals', s[i], 'and', g.condvalstack[i]);
                m = m && matchVals(computeBracket(s[i]), g.condvalstack[i]);
                console.log('new m is ' + m);
            }
            if(m) {
        ...
        default: { // section or other metadata directives
            // TODO: preprocess section directives (incl conditionals?)
            cont();
            break;
        }
        }
...
function loadLevelFromStateTarget(state,levelindex,target,randomseed) {	
    var leveldat = target;    
    g.curlevel=levelindex;
    g.curlevelTarget=target;
    if (leveldat.message===undefined) {
        if (levelindex=== 0){ 
...
function loadLevelFromTag(state, tag, randomseed) {
    if(typeof tag === typeof 5) {
        loadLevelFromState(state, tag, randomseed);
        return;
    } else if(typeof tag !== typeof '') {
        consoleError("Tried to load a tag '"+tag+"' which is not a string!");
        return;
    }
    for(var lix = 0; lix < state.levels.length; lix++) {
        if(state.levels[lix].action === 'section' && state.levels[lix].message === tag) {
            break;
        }
    }
    if(lix >= state.levels.length) {
        consoleError("Tried to load tag '" + tag + "' which doesn't exit");
    }
    loadLevelFromState(state, lix, randomseed);
}

function getLevelID(tag) {
    if(typeof tag === typeof 5) {
        return tag;
    } else if(typeof tag !== typeof '') {
        //consoleError("Tried to get the ID of a tag which is not a string!");
        return 0;
    }
    for(var lix = 0; lix < state.levels.length; lix++) {
        if(state.levels[lix].action === 'section' && state.levels[lix].message === tag) {
...
// this is /not/ just lix + n, because it skips non-levels (ish).
// as with everything goto-like, this will fail /horribly/ if you
// need it to parse conditionals etc.
// This is the correct decision because conditionals can evaluate arbitrary
// code.
// Anyways, it will work for anything else
// more precisely, it'll skip `n-1` /regular/ levels,
// and then skip all directives until it reaches a non-directive (i.e. 'message' or a regular level).
// for the purposes of this function, /section/ directives are regular levels.
function getNLevelsLater(lix, n, sec) {
    if(n == 0) {
        return lix;
    }
    for(var i = 0; i < n; i++) {
        do {
            lix++;
        } while(lix < state.levels.length && 'action' in state.levels[lix] && (!sec || state.levels[lix].action !== 'section'))
    }
    /*
    do {
        lix++;
    } while(lix < state.levels.length && 'action' in state.levels[lix] && state.levels[lix] && state.levels[lix].action !== 'section')
    */
    if(lix > state.levels.length) {
        lix = state.levels.length;
    }
    return lix;
}

function firstLevelFrom(lix, sec) {
    lix = getLevelID(lix);
    while(lix < state.levels.length && 'action' in state.levels[lix] /*&& state.levels[lix].action !== 'message'*/ && (!sec || state.levels[lix].action !== 'section')) {
        lix++;
    }
    if(lix > state.levels.length) {
        lix = state.levels.length;
...
function getSectionAndOffset(tag) {
    if(typeof tag !== typeof 5 && tag !== typeof '') {
        consoleError("Tried to get the offset of a nonsense level id!");
        return undefined;
    }
    var t = '';
    // first level has tag == 0 and we increment the offset before checking.
    var off = -1;
    if(typeof tag === typeof 5 && tag < 0) {
        return {tag:'', offset:tag};
    }
    for(var lix = 0; lix < state.levels.length; lix++) {
        off++;
        if(state.levels[lix].action === 'section') {
            t = state.levels[lix].message;
            off = 0;
            if(t === tag) {
                return {tag:t, offset: 0};
            }
            break;
        }
        if(lix === tag) {
            return {tag:t, offset:off};
        }
    }
    return {tag:t,offset:off+tag-lix+1};
}

function asfunc(v) {
    if(typeof v === typeof asfunc) {
        return v;
    } else if(v !== undefined && v.isfunc) {
        return v.asfunc;
    } else {
...
var bracketScopes = [];
// probably this should happen at compile time, but it's fine like this
function scopeOfVariable(s, preferloc) {
    if(s in g) {
        return g;
    } else if(s in consts) {
        return consts;
    } else if(s === 'NaN' || !isNaN(parseFloat(s))) {
        return undefined;
    } else {
        for(var i = 0; i < bracketScopes.length; i++) {
            if(s in bracketScopes[i]) {
                return bracketScopes[i];
            }
        }
        console.log('not in any scope');
        return preferloc ? bracketScopes[bracketScopes.length-1] : g;
    }
}


function derefString(s) {
    if(typeof s !== typeof '') {
        if(s instanceof UnevaluatedBracket) {
            consoleError("Sorry, but first-class functions are forbidden");
        }
        return s;
    }
    s = s.trim();
    console.log('deref ' + s);
    var v=scopeOfVariable(s);
    if(v === undefined) {
        v= (s === 'NaN' || !isNaN(parseFloat(s))) ? parseFloat(s) : undefined;
    } else {
        v = v[s];
    }
    //    var v = (s in g) ? g[s] : ((s in consts) ? consts[s] : parseFloat(s));
    return asfunc(v);
}


function computeBracket(s) {
    if(!(s instanceof UnevaluatedBracket)) {
        return s;
    } else if(s.v.length === 0) {
        return undefined;
    } else {
        bracketScopes.push({});
        var fs = [].concat(s.v);
...
function computeBracketed(s) {
    return computeBracket(new UnevaluatedBracket(s));
}
    ...
    case "restart":
        {
            ...
            tryPlayTitleSound();
            textMode=true;
            //if('noautosave' in state.metadata) {
            ...
            messageselected=false;
            titleMode = 0;
            if ((g.curlevel>0||g.curlevelTarget!==null)) {
                titleMode=1;
            }
            generateTitleScreen();
            break;
...
Rule.prototype.queueCommands = function() {
    ...
    for(var i=0;i<commands.length;i++) {
        ...
        if (command[0]==='wintype') {
            this.wintype=command[1];
        }
    }
};
...
function writeSave() {
    g.savelevel = g.curlevel;
    localStorage.removeItem(document.URL+'_checkpoint');
    localStorage[document.URL]=JSON.stringify(g);
//    localStorage[document.URL+'_typ']=JSON.stringify(gtyp);
}
...
function processInput(dir,dontDoWin,dontModify) {
    ...
    if (dir<=4) {
        ...
        if (dontModify && level.commandQueue.indexOf('wintype')>=0) {
            return true;
        }
	      ...
	      if (!winning) {
			      if (level.commandQueue.indexOf('checkpoint')>=0) {
		    	      if (verbose_logging) { 
				        ...
				        g.restartTarget=level4Serialization();
				        hasUsedCheckpoint=true;
                if('noautosave' in state.metadata) {
                    if(verbose_logging) {
                        var r = level.commandQueueSourceRules[level.commandQueue.indexOf('checkpoint')];
		    		            consolePrintFromRule('NOAUTOSAVE feature set, not writing to localstorage.',r);
                    }
                } else {
                    g.savelevel = g.curlevel;
                    writeSave();
                    /*
                    var backupStr = JSON.stringify(g.restartTarget);
                    localStorage[document.URL+'_checkpoint']=backupStr;
				            localStorage[document.URL]= g.curlevel;
                    */
                }
			      }
            if (level.commandQueue.indexOf('savegame')>=0) {
                // TODO: should savegame accept a destination name?
		    	      if (verbose_logging) { 
	    			        var r = level.commandQueueSourceRules[level.commandQueue.indexOf('savegame')];
		    		        consolePrintFromRule('SAVEGAME command executed, saving current state to localStorage.',r);
				        }
				        g.restartTarget=level4Serialization();
				        hasUsedCheckpoint=true;
                g.savelevel = g.curlevel;
...
function checkWin(dontDoWin) {
    ...
    // TODO: fix
	  if (level.commandQueue.indexOf('wintype')>=0) {
	    	var r = level.commandQueueSourceRules[level.commandQueue.indexOf('wintype')];
		    consolePrint(r === ' ' ? "Win Condition Satisfied" : "Win Condition '" + r.wintype + "' Satisfied");
		    if(!dontDoWin){
			      DoWin(r.wintype);
		    }
		    return;
	  ...
	  var won= false;
    for(var w in state.winconditions) {
        if (state.winconditions[w].length>0 && !won)  {
            var passed=true;
            for (var wcIndex=0;wcIndex<state.winconditions[w].length;wcIndex++) {
                var wincondition = state.winconditions[w][wcIndex];
	  ...
	  if (won) {
		    consolePrint(won === ' ' ? "Win Condition Satisfied" :"Win Condition '"+won+ "' Satisfied");
		    if (!dontDoWin){
			      DoWin(won);
		    }
	  }
...
function DoWin(w) {
    ...
    var ww = w.trim();
    g.lastWin = ww;
    if(g._beatlevel === undefined) {
        g._beatlevel = {};
    }
    if(g._beatlevel[g.curlevel] === undefined) {
        g._beatlevel[g.curlevel] = [w];
    } else if(g._beatlevel[g.curlevel].indexOf(w) < 0) {
        g._beatlevel[g.curlevel].push(w);
    }
...
function nextLevel(w) {
    againing=false;
	  messagetext="";
	  if (state && state.levels && (g.curlevel>state.levels.length) ){
		    g.curlevel=state.levels.length-1;
	  }
	  ...
	  if (titleScreen) {
		    ...
		    if (curlevelTarget!==null){
			      loadLevelFromStateTarget(state,g.curlevel,curlevelTarget);
		    } else {
			      loadLevelFromState(state,g.curlevel);
		    }
	  } else {
		    ...
		    if (g.curlevel<(state.levels.length-1))
		    {			
			      g.curlevel++;
			      textMode=false;
			      titleScreen=false;
			      ...
			      try{
				        if (!!window.localStorage && !('noautosave' in state.metadata)) {
	                  
					          localStorage.removeItem(document.URL);
					          localStorage.removeItem(document.URL+'_checkpoint');
                    localStorage.removeItem(document.URL+'_typ');
				        }
			      } catch(ex){
...
function goToTitleScreen(){
    againing=false;
	  ...
	  textMode=true;
	  doSetupTitleScreenLevelContinue();
	  titleSelection=(g.curlevel>0||curlevelTarget!==null)?1:0;
	  generateTitleScreen();
}
